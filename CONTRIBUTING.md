...
==========================
SapphireMC is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Sapphire will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Sapphire from
...
- A Java 17 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Sapphire requires JDK 17 to build, however, makes use of Gradle's
...
3. cd into `Sapphire-Server` for server changes, and `Sapphire-API` for API changes.

`Sapphire-Server` and `Sapphire-API` aren't git repositories in the traditional sense:

- `base` points to the unmodified source before Sapphire patches have been applied.
- Each commit after `base` is a patch.

## Adding Patches

Adding patches to Sapphire is very simple:

1. Modify `Sapphire-Server` and/or `Sapphire-API` with the appropriate changes;
...
Your commit will be converted into a patch that you can then PR into Sapphire.
...
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official SapphireMC repository.
...
1. If your PR creates new patches instead of modifying existing ones, in both the `Sapphire-Server` and `Sapphire-API` directories, ensure your newly-created patch is the last commit by either:
...
Remember that these changes will affect everyone who runs Sapphire, not just you
and your server.
...
All modifications to non-Sapphire files should be marked.

- Multi-line changes start with `// Sapphire start` and end with `// Sapphire end`;
- You can put a comment with an explanation if it isn't obvious, like this:
  `// Sapphire start - reason`.
    - The comments should generally be about the reason the change was made, what
      it was before, or what the change is.
    - Multi-line messages should start with `// Sapphire start` and use `/* Multi
      line message here */` for the message itself.
- One-line changes should have `// Sapphire` or `// Sapphire - reason`.

Here's an example of how to mark changes by Sapphire:

```java
entity.getWorld().dontbeStupid(); // Sapphire - was beStupid() which is bad
entity.getFriends().forEach(Entity::explode);
entity.a();
entity.b();
// Sapphire start - use plugin-set spawn
...
When submitting patches to Sapphire, we may ask you to add notes to the patch
...
If you add a message to your commit in the `Sapphire-Server`/`Sapphire-API`
...
to make future updates easier on ourselves, Sapphire tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Sapphire - OBFHELPER
// ...   
this.someMethod(fromX); // Sapphire
```
...
`SapphireConfig` or `SapphireWorldConfig` classes. Use `SapphireConfig` if a value
...
```java
if (!SapphireConfig.saveEmptyScoreboardTeams) {
```

It is often preferred that you use the fully qualified name for the
configuration class when accessing it, like so:
`io.sapphiremc.sapphire.SapphireConfig.valueHere`.  
If this is not done, a developer for Sapphire might fix that for you before
merging, but it's always nice if you make it a habit where you only need 1-2
lines changed.
### SapphireWorldConfig example
...
object:
```java
return this.level.sapphireConfig.useInhabitedTime ? this.inhabitedTime : 0;
```
...
By default, Sapphire (and upstream) only import files we make changes to. If you
would like to make changes to a file that isn't present in `Sapphire-Server`'s
...
1. Identify the name(s) of the file(s) you want to import.
    - A complete list of all possible file names can be found at
      `./Sapphire-Server/.gradle/caches/paperweight/mc-dev-sources/net/minecraft/`. You might find
