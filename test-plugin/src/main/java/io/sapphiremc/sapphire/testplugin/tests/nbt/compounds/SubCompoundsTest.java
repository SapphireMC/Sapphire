...
public class SubCompoundsTest implements NBTTest {
    ...
    public void test() throws Exception {
        ...
        if (cont.getCompound("invalide") != null) {
            throw new NBTException("An invalide compound did not return null!");
        }
        ...
        if (comp == null) {
            throw new NBTException("Wasn't able to get the NBTCompound!");
        }
        if (!comp.hasKey(STRING_TEST_KEY)) {
            throw new NBTException("Wasn't able to check a compound key!");
        }
        if (!(STRING_TEST_VALUE + "2").equals(comp.getString(STRING_TEST_KEY))
            ...
            throw new NBTException("One key does not equal the original compound value!");
        }
        ...
        if (!comp.getOrCreateCompound("someName").getString("test").equals("abc")) {
            throw new NBTException("getOrCreateCompound did not return the same compound!");
        }
    }
}
